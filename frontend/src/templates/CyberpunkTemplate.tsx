                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-black/40 p-8 md:p-12 border border-[#f000ff]/30 shadow-[inset_0_0_30px_rgba(240,0,255,0.1)] relative overflow-hidden" style={{ clipPath: 'polygon(0 0, 100% 0, 100% calc(100% - 30px), calc(100% - 30px) 100%, 0 100%)' }}>
                        ...
                        <div className="lg:col-span-8 flex flex-col gap-4 relative z-10">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                                {pd.contact?.email && <div className="flex items-center gap-3 text-sm text-gray-400"><Mail className="w-4 h-4 text-[#00ffff]" /> <span className="hover:text-white transition-colors cursor-crosshair">{pd.contact.email}</span></div>}
                                {pd.contact?.phone && <div className="flex items-center gap-3 text-sm text-gray-400"><Phone className="w-4 h-4 text-[#00ffff]" /> <span className="hover:text-white transition-colors cursor-crosshair">{pd.contact.phone}</span></div>}
                                {pd.contact?.location && <div className="flex items-center gap-3 text-sm text-gray-400"><MapPin className="w-4 h-4 text-[#00ffff]" /> <span className="hover:text-white transition-colors cursor-crosshair">{pd.contact.location}</span></div>}
                                {pd.contact?.linkedin && <div className="flex items-center gap-3 text-sm text-gray-400"><Globe className="w-4 h-4 text-[#00ffff]" /> <a href={pd.contact.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors hover:underline cursor-crosshair">LINK_NET</a></div>}
                            </div>
                        </div>
                            ...
                            {pd.projects.map((proj: any, i: number) => (
                                <div key={i} className="group flex flex-col bg-black border border-[#00ffff]/30 hover:border-[#00ffff] p-6 transition-all hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(0,255,255,0.2)]">
                                    <div className="flex items-center justify-between mb-4">
                                        <h3 className="text-xl font-bold text-[#f000ff] uppercase">{proj.name}</h3>
                                        {proj.link ? (
                                            <a href={proj.link} target="_blank" rel="noopener noreferrer" className="text-[#00ffff] hover:text-white transition-colors">
                                    ...
                                    {proj.technologies && (
                                        <div className="flex items-center gap-2 mb-4">
                                            ...
                                            <span className="text-xs text-[#ffeb3b] font-bold tracking-widest">{proj.technologies}</span>
                                        </div>
                                    )}
                            ...
                            {pd.education.map((edu: any, i: number) => (
                                <div key={i} className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#111] border border-gray-800 p-6 hover:border-[#ffeb3b] transition-colors">
                                    ...
                                    <div className="text-left md:text-right border-l-2 md:border-l-0 md:border-r-2 border-[#ffeb3b] pl-4 md:pl-0 md:pr-4">
                                        <p className="text-sm text-[#00ffff] font-bold tracking-widest">[ {edu.duration} ]</p>
